...
func TestRouter(t *testing.T) {
	...
	r.Config("/index", func(w http.ResponseWriter, r *http.Request) {
	...
	require.Equal(t, "/index", handlers[0].GetPatten())
	require.Equal(t, "index page", handlers[0].GetDesc())

	// When config a testHandler with pattern "/test1", and try to find the new handler from the router
	testHandler := func(w http.ResponseWriter, r *http.Request) {}
	r.Config("/test", testHandler)
	...
	require.Equal(t,
		...
		runtime.FuncForPC(reflect.ValueOf(handler.GetHandler()).Pointer()).Name(),
	)

	// When start a http server with the router, and send a http GET request to access index page
	ln := mustListenAndServe(t, r)
	t.Cleanup(func() { ln.Close() })
	resp, err := http.Get(fmt.Sprintf("http://%s%s", ln.Addr(), "/index"))
	require.Nil(t, err, "http.Get error: %+v", err)
	...
	resp, err = http.Get(fmt.Sprintf("http://%v%s", ln.Addr(), "/nonexistent-resource"))
	require.Nil(t, err, "http.Get error: %+v", err)
	body, err = io.ReadAll(resp.Body)
	resp.Body.Close()
...
func TestRouter_ServeHTTP(t *testing.T) {
	t.Run("panic but recovered", func(t *testing.T) {
		...
		ln := mustListenAndServe(t, r)
		t.Cleanup(func() { ln.Close() })
		resp, err := http.Get(fmt.Sprintf("http://%v%s", ln.Addr(), "/index"))
		require.Nil(t, err, "http.Get error: %+v", err)
		body, err := io.ReadAll(resp.Body)
		resp.Body.Close()
...
func mustListenAndServe(t *testing.T, r Router) net.Listener {
	l := mustListenTCP(t)
	go func() {
