...
![timeout_control](https://git.woa.com/trpc-go/trpc-go/raw/master/.resources/user_guide/timeout_control/timeout_control.png)

## 3 超时控制实现

tRPC-Go 的超时控制都是`基于 context 能力`实现的。
context 是请求上下文的意思，是所有 rpc 接口的第一个参数，可以设置超时，取消。所以要实现 tRPC-Go 的超时控制，所有的 rpc 调用都必须一路携带请求入口的 ctx。`必须注意：超时只有通过 ctx 才能控制。`
...
可以参考下笔记：【trpc-go 服务超时时间为什么会不生效？】<http://mk.woa.com/note/7463>
...
请参考 tRPC-Go 错误码手册 [FAQ](https://iwiki.woa.com/p/4008319150#6faq) 中关于超时的相关问题。

更多信息可以阅读相关提案：[超时](https://git.woa.com/trpc/trpc-proposal/blob/master/A16-timeout.md)
