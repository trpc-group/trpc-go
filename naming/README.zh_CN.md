...
在 tRPC-Go 中，`register` 包定义了服务端的注册规范，`discovery`、`servicerouter`、`loadbalance`、`circuitebreaker` 则一起组成 `selector` 包并定义了客户端的服务发现规范。
...
tRPC-Go 支持[北极星](https://git.woa.com/trpc-go/trpc-naming-polaris)，可以根据服务名进行服务发现。假如业务方在调用时需要设置 Target，会根据 target 的 endpoint 去进行服务发现。
