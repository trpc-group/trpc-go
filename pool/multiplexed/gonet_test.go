...
func beginUDPServer(t *testing.T, _ func(net.Conn), _ *tls.Config) (net.Addr, context.CancelFunc) {
...
func TestBasic(t *testing.T) {
	testcases := []struct {
		...
		{
			name:     "TCP_Gonet",
			begin:    beginTCPServer,
			dialFunc: NewDialFunc(WithDropFull(), WithSendingQueueSize(100)),
		},
		{name: "UDP_Gonet", begin: beginUDPServer, dialFunc: NewDialFunc()},
	}
...
func TestGetConnection(t *testing.T) {
	testcases := []struct {
		...
		{name: "TCP_Gonet", begin: beginTCPServer, dialFunc: NewDialFunc()},
		{name: "UDP_Gonet", begin: beginUDPServer, dialFunc: NewDialFunc()},
	}
	for _, tt := range testcases {
		t.Run(tt.name, func(t *testing.T) {
...
func TestDial(t *testing.T) {
	testcases := []struct {
		...
		{name: "DialSucceed_Gonet", dialFunc: NewDialFunc(), wantErr: nil},
		{
			name: "DialTimeout",
			dialFunc: func(_ FrameParser, opts *connpool.DialOptions) (Conn, error) {
...
func TestServerCloseConnAfterAccept(t *testing.T) {
	testcases := []struct {
		name     string
		dialFunc DialFunc
	}{
		{name: "TCP_Gonet", dialFunc: NewDialFunc()},
	}
	for _, tt := range testcases {
		t.Run(tt.name, func(t *testing.T) {
...
func TestDecodeFail(t *testing.T) {
	testcases := []struct {
		...
		{name: "TCP_Gonet", begin: beginTCPServer, dialFunc: NewDialFunc(), wantErrMsg: "decode fail"},
