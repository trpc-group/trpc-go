...
func Test_newPayload(t *testing.T) {
	...
	_, err := newPayload(testpb.PayloadType_COMPRESSABLE, invalidLength)
	...
	require.Equal(t, retUnsupportedPayload, errs.Code(err))
	require.Contains(t, err.Error(), fmt.Sprintf("unsupported payload type: %d", testpb.PayloadType_RANDOM))

	_, err = newPayload(testpb.PayloadType_COMPRESSABLE, int32(1))
	require.Nil(t, err)
}
